...
########### MODIFIED: MICHAŁ GENEJA, WARSAW 2025 ########################
#########################################################################
...
chrs = {0: 'chr1', 1: 'chr2', 2: 'chr3', 3: 'chr4', 4: 'chr5', 5: 'chrC', 6: 'chrM'}

chrom_lengths_array = np.array([0, 30427671, 19698289, 23459830, 18585056, 26975502, 154478, 367808])

chrom_sizes = {'chr1': 30427671, 'chr2': 19698289, 'chr3': 23459830,
               'chr4': 18585056, 'chr5': 26975502, 'chrC': 154478, 'chrM': 367808}
...
def get_coordinates_mm(mm_vec):
    """
    It returns the coordinate matrix V (N,3) of a .pdb file.
    The main problem of this function is that coordinates are not always in
    the same column position of a .pdb file. Do changes appropriately,
    in case that the data aren't stored correctly.

    Input:
    file (Openmm Quantity): an OpenMM vector of the form
...
def import_bed(bed_file, N_beads, coords=None, chrom=None, save_path='', shuffle=False, seed=0, n_chroms=5):
    # Load compartment dataset
    np.random.seed(seed)
    comps_df = pd.read_csv(bed_file, header=None, sep='\t')

    # Find maximum coordinate of each chromosome
    print('Cleaning and transforming sub-compartments dataframe...')
    ...
    chrom_ends = np.cumsum(np.insert(chrom_lengths_array[1:][chrom_idxs], 0, 0)) if chrom is None else np.array(
        [0, chrom_sizes[chrom]])
...
def import_mns_from_bedpe(bedpe_file, N_beads, coords=None, chrom=None, threshold=0, min_loop_dist=2,
                          path='', down_prob=1.0, shuffle=False, seed=0, n_chroms=5):
    # Import loops
    np.random.seed(seed)
    ...
    if shuffle: np.random.shuffle(chrom_idxs)
    if chrom is not None:
        loops = loops[(loops[0] == chrom) & (loops[1] > coords[0]) & (loops[2] < coords[1]) & (loops[4] > coords[0]) & (
                loops[5] < coords[1])].reset_index(drop=True)
    chrom_ends = np.cumsum(np.insert(chrom_lengths_array[1:][chrom_idxs], 0, 0)) if chrom is None else np.array(
        [0, chrom_sizes[chrom]])
    ...
    if chrom is not None:
        loops[1], loops[2], loops[4], loops[5] = loops[1] - coords[0], loops[2] - coords[0], loops[4] - coords[0], \
                                                 loops[5] - coords[0]
    loops[1], loops[2], loops[4], loops[5] = loops[1] // resolution, loops[2] // resolution, loops[4] // resolution, \
                                             loops[5] // resolution
    ...
    mns, cs = np.vstack((loops['ms'].values[counts > threshold], loops['ns'].values[counts > threshold])), counts[
        counts > threshold]
...
def import_bw(bw_path, N_beads, coords=None, chrom=None, viz=False, binary=False, path='', norm=False, shuffle=False,
              seed=0, n_chroms=5):
    """
    Imports .BigWig data and outputs compartments.

    It assumes that higher signal corresponds to B compartment.
    ...
    if norm: genomewide_signal = (genomewide_signal - np.mean(genomewide_signal) + 3 * np.std(
        genomewide_signal)) / np.std(genomewide_signal)
    ...
    if viz:
        ...
        plt.fill_between(xax, genomewide_signal, where=(genomewide_signal > np.mean(genomewide_signal)), alpha=0.50,
                         color='purple')
        plt.fill_between(xax, genomewide_signal, where=(genomewide_signal < np.mean(genomewide_signal)), alpha=0.50,
                         color='orange')
        ...
        plt.ylim((np.mean(genomewide_signal) - np.std(genomewide_signal),
...
def chrom_sort_key(chrom):
    ...
    val = match.group(1)
    return int(val) if val.isdigit() else {'C': 6, 'M': 7}[val]
...
def get_eigenvector(eigenvec_tsv, N_beads, chrom=None, region=None, viz=False):
    ...
    if chrom is not None:
        ...
        for i in range(n_chroms):
            eigenvector[eigenvector['chrom'] == chroms[i]]['start'] = eigenvector[eigenvector['chrom'] == chroms[i]][
                                                                          'start'] + chrom_ends[i]
            eigenvector[eigenvector['chrom'] == chroms[i]]['end'] = eigenvector[eigenvector['chrom'] == chroms[i]][
                                                                        'end'] + chrom_ends[i]
