...
def plot_projection(struct_3D, Cs, save_path):
    ...
    plt.title('Sub-compartment 2D Density Plot')
    ...
    plt.title('Sub-compartment Density Plot')
...
def viz_structure(V, colors=None, r=0.1, cmap='coolwarm', save_path=None):
    ...
    plotter.add_mesh(polymer, smooth_shading=True, cmap=cmap, scalars="colors" if colors is not None else None,
...
def viz_chroms(sim_path, r=0.1, comps=True):
    ...
    if comps is not None: viz_structure(V, comps[:len(V)], cmap='coolwarm', r=r,
