...
class NucleosomeInterpolation:
    ...
    def move_structure_to(self, struct, p0, p1, p2):
        ...
        w_x = make_unit(p2 - p1)
        w_y = make_unit(get_perp_component(p1 - p0, w_x))
        w_z = make_unit(np.cross(w_x, w_y))
    ...
    def single_bead_nucgenerator(self, start_point, end_point, num_nucleosomes, prev_zigzag_vec=None, turns=1.65,
        ...
        segment_vector_norm = make_unit(segment_vector)
        ...
        if prev_zigzag_vec is None:
            zigzag_vec1 = make_unit(get_perpendicular(segment_vector))
        else:
            zigzag_vec1 = make_unit(get_perp_component(prev_zigzag_vec, segment_vector))
            if all(v == 0 for v in zigzag_vec1):
                zigzag_vec1 = make_unit(get_perpendicular(segment_vector))
        zigzag_vec2 = make_unit(np.cross(zigzag_vec1, segment_vector))
        phi = 0
...
def main():
    ...
    bw = import_bw('/home/skorsak/Data/encode/ATAC-Seq/ENCSR637XSC_GM12878/ENCFF667MDI_pval.bigWig',
                   len(V))  # Mock self.signal array
