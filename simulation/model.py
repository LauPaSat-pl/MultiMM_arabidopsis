...
class MultiMM:
    def __init__(self, args):
        ...
        if args.GENE_ID is None and args.GENE_NAME is None and args.LOC_START is None: os.makedirs(
        ...
        if (args.GENE_TSV is not None) and (str(args.MODELLING_LEVEL).lower() == 'gene'):
            if args.GENE_ID is not None and str(args.GENE_ID).lower() != 'none' and str(args.GENE_ID).lower() != '':
                ...
                self.gene_start, self.gene_end = ((gene_coords[0] - coords[0]) * self.args.N_BEADS) // (
                        coords[1] - coords[0]), ((gene_coords[1] - coords[0]) * self.args.N_BEADS) // (
                                                         coords[1] - coords[0])
                print(f'We model the region {coords[0]}-{coords[1]} of chrom {chrom} of the gene {args.GENE_ID}.\n')
            elif args.GENE_NAME and str(args.GENE_NAME).lower() != 'none':
                print('Gene name:', args.GENE_NAME)
                chrom, coords, gene_coords = get_gene_region(gene_tsv=args.GENE_TSV, gene_name=args.GENE_NAME,
                                                             window_size=args.GENE_WINDOW)
                self.gene_start, self.gene_end = ((gene_coords[0] - coords[0]) * self.args.N_BEADS) // (
                        coords[1] - coords[0]), ((gene_coords[1] - coords[0]) * self.args.N_BEADS) // (
                                                         coords[1] - coords[0])
        ...
        if args.COMPARTMENT_PATH is not None and str(args.COMPARTMENT_PATH).lower() != 'none' and str(
            ...
            if args.COMPARTMENT_PATH.lower().endswith('.bed'):
                self.Cs, self.chr_ends, self.chrom_idxs = import_bed(bed_file=args.COMPARTMENT_PATH,
                                                                     ...
                                                                     coords=coords, save_path=self.save_path,
        ...
        if str(args.LOOPS_PATH).lower().endswith('.bedpe'):
            self.ms, self.ns, self.ds, self.chr_ends, self.chrom_idxs = import_mns_from_bedpe(
                bedpe_file=args.LOOPS_PATH, N_beads=self.args.N_BEADS, coords=coords, chrom=chrom, path=self.save_path,
        ...
        if self.args.CHROM == '': write_chrom_colors(self.chr_ends, self.chrom_idxs,
    ...
    def add_harmonic_bonds(self):
        ...
        for i in range(self.system.getNumParticles() - 1):
            if (i not in self.chr_ends): self.bond_force.addBond(i, i + 1, self.args.POL_HARMONIC_BOND_R0,
    ...
    def add_stiffness(self):
        ...
        for i in range(self.system.getNumParticles() - 2):
            if (i not in self.chr_ends) and (i not in self.chr_ends - 1):
                self.angle_force.addAngle(i, i + 1, i + 2, self.args.POL_HARMONIC_ANGLE_R0,
    ...
    def initialize_simulation(self):
        if self.args.BUILD_INITIAL_STRUCTURE:
            ...
            comp_mode = 'compartments' if np.all(self.Cs is not None) and len(
                np.unique(self.Cs)) <= 3 else 'sub-compartments'
            if np.all(self.Cs is not None): write_cmm(self.Cs,
            ...
            pdb_content = build_init_mmcif(n_dna=self.args.N_BEADS, chrom_ends=self.chr_ends,
                                           path=self.save_path + 'metadata/', curve=self.args.INITIAL_STRUCTURE_TYPE,
                                           scale=(self.radius1 + self.radius2) / 2)
            print('---Done!---')
        self.pdb = PDBxFile(
            self.save_path + 'metadata/MultiMM_init.cif') if self.args.INITIAL_STRUCTURE_path is None or build_init_mmcif else PDBxFile(
            self.args.INITIAL_STRUCTURE_PATH)
        ...
        match args.SIM_INTEGRATOR_TYPE:
            ...
            case 'langevin':
                self.integrator = mm.LangevinIntegrator(self.args.SIM_TEMPERATURE, self.args.SIM_FRICTION_COEFF,
                                                        self.args.SIM_INTEGRATOR_STEP)
            case 'variable_langevin':
                self.integrator = mm.VariableLangevinIntegrator(self.args.SIM_TEMPERATURE, self.args.SIM_FRICTION_COEFF,
                                                                self.SIM_ERROR_TOLERANCE)
            case 'amd':
                self.integrator = mm.amd.AMDIntegrator(self.args.SIM_INTEGRATOR_STEP, self.args.SIM_AMD_ALPHA,
                                                       self.args.SIM_AMD_E)
            case 'brownian':
                self.integrator = mm.BrownianIntegrator(self.args.SIM_TEMPERATURE, self.args.SIM_FRICTION_COEFF,
                                                        self.args.SIM_INTEGRATOR_STEP)
    ...
    def min_energy(self):
        ...
        PDBxFile.writeFile(self.pdb.topology, self.state.getPositions(),
    ...
    def run_md(self):
        self.simulation.reporters.append(
            StateDataReporter(stdout, self.args.SIM_SAMPLING_STEP, step=True, totalEnergy=True, kineticEnergy=True,
                              potentialEnergy=True, temperature=True, separator='\t'))
        self.simulation.reporters.append(DCDReporter(self.save_path + 'metadata/MultiMM_annealing.dcd',
    ...
    def nuc_interpolation(self):
        ...
        start = time.time()
        nuc_interpol = NucleosomeInterpolation(get_coordinates_cif(self.save_path + 'model/MultiMM_minimized.cif'),
                                               self.atacseq, self.args.MAX_NUCS_PER_BEAD, self.args.NUC_RADIUS,
                                               self.args.POINTS_PER_NUC, self.args.PHI_NORM)
        ...
        elapsed = end - start
        print(
            f'Nucleosome interpolation finished successfully in {elapsed // 3600:.0f} hours, {elapsed % 3600 // 60:.0f} minutes and  {elapsed % 60:.0f} seconds.')

    def set_radiuses(self):
        self.radius1 = (self.args.N_BEADS / 50000) ** (1 / 3) if self.args.SC_RADIUS1 is None else self.args.SC_RADIUS1
        self.radius2 = 3.5 * (self.args.N_BEADS / 50000) ** (
                1 / 3) if self.args.SC_RADIUS2 is None else self.args.SC_RADIUS2
    ...
    def make_plots(self):
        is_gw = all((self.args.GENE_ID is None, self.args.GENE_NAME is None, self.args.LOC_START is None,
                     self.args.LOC_END is None))
        is_comp = np.any(self.Cs is not None).item()
        if is_gw:
            if is_comp: plot_projection(get_coordinates_mm(self.state.getPositions()), self.Cs,
            ...
            for i in range(len(self.chr_ends) - 1):
                ...
                viz_structure(V, r=0.2, cmap='coolwarm',
                              save_path=self.save_path + f'plots/chromosomes/{chrs[self.chrom_idxs[i]]}_minimized_structure.png')
        else:
            if args.GENE_ID is not None or args.GENE_NAME is not None:
                save_chimera_cmd(self.gene_start, self.gene_end, self.args.N_BEADS,
                ...
                viz_gene_structure(V, self.gene_start, self.gene_end, r=0.2, cmap='coolwarm',
            ...
            if is_comp: viz_structure(V, self.Cs[:len(V)], cmap='coolwarm', r=0.2,
    ...
    def run(self):
        ...
        self.min_energy()
        if args.GENE_ID is None and args.GENE_NAME is None and args.LOC_START is None: self.save_chromosomes()
        ...
        if self.args.NUC_DO_INTERPOLATION and args.ATACSEQ_PATH is not None:
